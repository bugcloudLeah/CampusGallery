...
const HARDHAT_DEPLOYMENTS_ROOT = path.resolve(ROOT, "campusgallery-hardhat", "deployments");
...
function readDeployment(network) {
  ...
  const file = path.resolve(dir, "CampusGallery.json");
...
function main() {
  ...
  const abiTs = `export const CampusGalleryABI = {\n  abi: ${JSON.stringify(gg.abi, null, 2)}\n};\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "CampusGalleryABI.ts"), abiTs);
  ...
  const addrTs = `export const CampusGalleryAddresses = ${JSON.stringify(addresses, null, 2)} as const;\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "CampusGalleryAddresses.ts"), addrTs);
