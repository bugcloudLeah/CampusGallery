...
export function useGallery(parameters: {
  ...
  const contractAddress = useMemo(() => {
    ...
    const entry = (CampusGalleryAddresses as any)[chainId?.toString()];
  ...
  const refreshArtworks = useCallback(async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, CampusGalleryABI.abi, rp);
  ...
  const like = useCallback(async (id: number) => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, CampusGalleryABI.abi, s);
      const tx = await contract.applaudPainting(id);
      await tx.wait();
  ...
  const vote = useCallback(async (id: number, category: string) => {
    ...
    try {
      ...
      const s = await bp.getSigner();
      const contract = new ethers.Contract(contractAddress, CampusGalleryABI.abi, s);
      const tx = await contract.endorsePainting(id, category);
      await tx.wait();
  ...
  const mockUpload = useCallback(async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, CampusGalleryABI.abi, s);
      const tags = ["校园", "速写"];
      const categories = ["campus-classic"];
      const tx = await contract.submitPainting("Untitled - 校园", "ipfs://descHash", "ipfs://fileHash", tags, categories);
      await tx.wait();
      await refreshArtworks();
    } catch (e: any) {
